...
| --- | --- | --- |
| `npm run smoke:production-tenant-readiness-prereqs` | Before running the full production tenant smoke | Loads `.env.local` for missing shell values, reports missing env names only, classifies DB host as local/non-local without printing the host, checks that Clerk credentials match the target environment, and exits non-zero if required prerequisites are missing. |
...
| `npm run smoke:production-intake-profile` | After deploying intake-prioritization changes to production | Targeted live app read/write smoke. Creates a temporary Clerk org, signs in with the smoke user, submits onboarding intake through the production UI, verifies `org_intake_profiles` and seeded control statuses in the production DB, checks dashboard/controls behavior, then deletes the smoke org/data. |
| `npm run check:production-migration-drift` | Before relying on production schema or as a CI/pre-deploy gate | Read-only check. Reads `lib/db/migrations/meta/_journal.json`, queries `drizzle.__drizzle_migrations`, infers missing migration tags by journal order, refuses local DB URLs by default, and exits non-zero if production is behind/ahead. Prefer `PRODUCTION_DATABASE_URL_UNPOOLED` or `DATABASE_URL_UNPOOLED`; never print credentials. |
| `npm run lint:copy:intake-prioritization` | After editing intake prioritization copy | Soft copy-risk lint. Checks tracked intake-prioritization strings for compliance/certification/legal-advice overclaims. If `OPENAI_API_KEY` is present or `--openai` is passed, also asks OpenAI for conservative human-review flags. Use `npm run lint:copy:intake-prioritization:strict` to fail on blocker findings. |
| `npm run test:e2e:intake-prioritization` | Before approving intake prioritization behavior or after changing controls/dashboard UI | Playwright smoke for controls default in-scope filter, explicit out-of-scope/not-applicable filter, dashboard no-intake rendering, and no obvious compliance-implying UI copy. Local runs use fallback/no-auth states; staging runs should use an authenticated storage state/session if full tenant data coverage is required. |
| `SMOKE_LIVE_OPENAI_QUESTIONNAIRE=true npm run smoke:production-tenant-readiness` | Proving buyer-facing authenticated production readiness | Requires production/non-local `DATABASE_URL`, Clerk smoke user credentials, Blob token, and live questionnaire generation opt-in. When `AUTH_PRIMARY_FLOW_BASE_URL` targets `splnit.eu`, the smoke also requires live Clerk keys (`pk_live_` / `sk_live_`); test Clerk keys only work against a non-production deployment wired to that same Clerk instance. Creates and cleans up smoke org/data, exercises authenticated app surfaces, and may attempt mailbox delivery only when mailbox env is present. |

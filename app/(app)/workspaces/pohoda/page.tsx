...
function buildDemoProgress(): WorkspaceProgress {
  ...
  return {
    ...
    completedControls: 0,
    totalControls: pohodaWorkspace.layers.flatMap((l) => l.controls).length,
    layers,
  };
}

async function loadPohodaProgress(): Promise<{
  ...
  try {
    const [progress, comments] = await Promise.all([
      getWorkspaceProgress(session.orgId, pohodaWorkspace),
    ...
    <section className="space-y-6">
      ...
      <div className="rounded-lg border border-border bg-white p-5 shadow-xs">
        <div className="flex flex-col gap-4 md:flex-row md:items-center">
          <span className="grid h-14 w-14 place-items-center rounded-lg bg-red-600 text-sm font-bold text-white">
            PO
          </span>
          <div>

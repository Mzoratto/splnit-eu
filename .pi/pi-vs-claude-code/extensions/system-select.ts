 ...
 *
 * Scans project-local .pi/agents/ for agent definition .md files.
 *
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		const dirs: [string, string][] = [[join(cwd, ".pi", "agents"), ".pi"]];
		...
		if (allAgents.length > 0) {
			notifyLines.push(`Loaded ${allAgents.length} project-local agents (${loadedSources})`);
		}
	...
	pi.registerCommand("system", {
		description: "Select a system prompt from project-local agents",
		handler: async (_args, ctx) => {
			if (allAgents.length === 0) {
				ctx.ui.notify("No project-local agents found in .pi/agents/*.md", "warning");
				return;
			}

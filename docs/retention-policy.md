...
| Questionnaire AI prompts/outputs | Customer questions, controls, policies, evidence references sent to Anthropic. | Keep generated answers as customer workspace data; vendor prompt retention depends on vendor terms/settings. | App stores outputs according to questionnaire workflow; Anthropic use is environment-dependent. | Confirm Anthropic data-retention setting/terms and whether customers must opt in. |

...
| Questionnaire AI prompts/outputs | Customer questions, controls, policies, evidence references sent to the configured AI provider. Anthropic is the only supported provider today. | Keep generated answers as customer workspace data; vendor prompt retention depends on vendor terms/settings. | App stores outputs as generated artifacts; provider calls require `QUESTIONNAIRE_AI_ENABLED=true`, `QUESTIONNAIRE_AI_PROVIDER=anthropic`, and `ANTHROPIC_API_KEY`. | Confirm Anthropic data-retention setting/terms and whether customers must opt in before enabling AI use. |

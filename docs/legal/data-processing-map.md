...
| NÚKIB registration preparation artifacts | Processor for customer-entered registration-prep content; controller for Splnit operational metadata where needed. | Create and retrieve a design-partner/feedback-beta preparation artifact for Czech NÚKIB registration filing prep. Generate a PDF on demand for customer review. Official filing remains on the NÚKIB portal. This is not legal advice, a production-ready filing service, or a guarantee of NÚKIB acceptance. | Customer organisation representatives and infrastructure/contact data: IČO; organisation/service description; statutory, technical, and security contacts; email, phone, role, and position; domains; IP ranges/CIDR blocks; cross-border dependencies; generated PDF metadata such as artifact ID, title, preparation timestamp, creator user ID, PDF filename, and HTTP download metadata. | Neon stores the generated artifact content in `generated_artifacts`; Vercel runtime handles form/API/PDF rendering; Vercel logs may receive request/error metadata, route paths, status codes, timings, and authenticated user/org identifiers depending on platform logging. Blob is not currently used by this route because the PDF is generated on demand from Neon-stored content and returned directly to the authenticated user. | Customer-controlled during contract; delete/return after service end or legal hold under the generated-artifact/workspace retention process. Avoid collecting real customer network topology before retention/export/deletion expectations are confirmed with the design partner. | Confirm production Neon region for `generated_artifacts`; confirm Vercel runtime region and log/data handling for this route; confirm whether any non-EU or US-controlled infrastructure/processors are involved. Do not state or imply EU-only residency until verified. |
...
| Questionnaire AI | Processor for customer-provided questionnaire context; controller for service operation logs where applicable. | Draft answers grounded in customer controls, policies, evidence references, and reviewed legal citations. | Customer questionnaire questions, organisation name/plan, control summaries, evidence summaries/metadata, policy metadata, reviewed citation metadata, and generated answers. Raw uploaded evidence files, secrets, credentials, and special-category personal data should not be sent. | OpenAI when `QUESTIONNAIRE_AI_ENABLED=true` and `OPENAI_API_KEY` are configured, Neon, Vercel. Provider registry currently supports OpenAI only. | Generated answers are stored as customer workspace generated artifacts; provider prompt retention depends on OpenAI account terms/settings. Outputs default to draft and require human review before external use. | Confirm OpenAI DPA/data-retention controls, training/use-of-inputs settings, transfer mechanism, support/log retention, and customer opt-in wording before broad customer use. |
...
- NÚKIB registration PDFs are generated on demand from Neon-stored generated artifact content and returned with private/no-store response headers; Blob storage is not used for the current NÚKIB route.
...
- Production data-residency statement for NÚKIB registration artifacts: Neon production region, Vercel runtime region/log handling, transfer mechanisms, and whether the path can truthfully be described as EU-only. Current guardrail: do not claim EU-only until verified.

...
| Auditor export | Comparison says included. | `/api/audit-log/export`, `/api/exports/workspace/archive`, `/api/exports/evidence-metadata`, `/api/exports/workspace`. | Audit/evidence/workspace export data generated from DB. | Authenticated and scoped; private/no-store where applicable. | Production-smoked for audit CSV and workspace ZIP; see `docs/export-endpoint-audit.md`. | Working for current buyer-facing export paths; do not claim generic vendor/risk/incident CSV endpoints. |

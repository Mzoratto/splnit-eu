...
- This verifies protected-route enforcement, sign-in/signup rendering, production deploy health, prerequisite presence for the second auth-smoke attempt, and cleanup of the temporary Clerk env file only. It does **not** verify authenticated tenant rendering for `/evidence`, `/questionnaires`, `/integrations`, `/trust-center`, `/vendors/[vendorId]`, or production email delivery.
...
1. **Authenticated production tenant smoke.** Sign in with a real production test tenant and verify `/evidence`, `/questionnaires`, questionnaire review/save/export gating, `/integrations`, vendor questionnaire delivery-status panel, and Trust Center admin save/toggle/request flows render without browser console errors. Current 2026-05-09 smoke proves signed-out redirects, Clerk sign-in/signup rendering, public Trust Center routes, source/local smokes, and prerequisite presence for a second auth-smoke attempt; the actual auth-smoke command was denied by the local tool approval layer, so tenant rendering remains unproven.
...
- Authenticated production tenant smoke for `/evidence`, `/questionnaires`, `/integrations`, `/trust-center`, `/vendors/[vendorId]`, and the questionnaire workbench/save flow. The 2026-05-09 browser pass verified signed-out redirects to Clerk sign-in with zero console errors; a later local attempt confirmed required Clerk/DB/Blob env presence, but the smoke command itself was denied by the local tool approval layer and did not run.

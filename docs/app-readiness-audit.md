...
- This verifies protected-route enforcement, sign-in/signup rendering, production deploy health, local `.env.local` prerequisite loading, successful production smoke setup/cleanup, Vercel env pull cleanup, and source availability of the reusable production tenant readiness smoke. It does **not** verify authenticated tenant rendering for `/evidence`, `/questionnaires`, `/integrations`, `/trust-center`, `/vendors/[vendorId]`, or production email delivery because production Clerk MFA/second-factor policy blocked the smoke browser session before route rendering.
...
- Authenticated production tenant smoke for `/evidence`, `/questionnaires`, `/integrations`, `/trust-center`, `/vendors/[vendorId]`, and the questionnaire workbench/save flow. The reusable secondary-surface smoke command is now `npm run smoke:production-tenant-readiness`; it now reads local `.env.local` values for missing shell env and reached production Clerk sign-in with required env present, but production Clerk returned `needs_second_factor` for the throwaway smoke user before route rendering.

...
- A later 2026-05-09 Vercel production env pull used a non-overwriting temp file, parsed it without printing values, and deleted it afterward. It confirmed `DATABASE_URL` and `BLOB_READ_WRITE_TOKEN` values are present, while `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` and `CLERK_SECRET_KEY` exist as production env names but currently pull as empty values, so `readyForTenantSmoke=false` and the authenticated tenant smoke still cannot run from this shell.
- A reusable authenticated secondary-surface production smoke now exists as `npm run smoke:production-tenant-readiness`. It creates a throwaway Clerk user/org, seeds a smoke vendor questionnaire and public Trust Center settings/request, signs into `https://splnit.eu` through Clerk testing tokens, renders `/dashboard`, `/evidence`, `/integrations`, provider detail pages, `/trust-center`, `/vendors`, `/vendors/[vendorId]`, `/questionnaires`, the public Trust Center access URL, and the vendor assessment token URL, then deletes the smoke Clerk/org/database rows. It emits redacted JSON only and gates the optional mailbox send on `RESEND_API_KEY`, `RESEND_FROM`, and `SMOKE_RECIPIENT_EMAIL`.
- Production email delivery is intentionally pending: `vercel env ls production` does not show `RESEND_API_KEY` or `RESEND_FROM`, and Microsoft 365 mailbox setup is not complete, so vendor questionnaire mailbox smoke must not be claimed.
- This verifies protected-route enforcement, sign-in/signup rendering, production deploy health, prerequisite presence for the second auth-smoke attempt, cleanup of the temporary Clerk env file, Vercel env pull cleanup, and source availability of the new reusable production tenant readiness smoke. `npm run smoke:production-tenant-readiness-prereqs` now provides a redacted readiness check; on this machine it reports `DATABASE_URL` and `BLOB_READ_WRITE_TOKEN` present, while the Vercel-pulled `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` and `CLERK_SECRET_KEY` names currently have empty values and Resend/smoke-recipient env is missing. It does **not** verify authenticated tenant rendering for `/evidence`, `/questionnaires`, `/integrations`, `/trust-center`, `/vendors/[vendorId]`, or production email delivery until the live smoke runs successfully against usable production secrets.
...
| Integrations hub | `/integrations` | partial | `getIntegrationsHubData`; provider config | Provider cards without DB; Google Workspace is disabled as coming soon | Localized descriptions and connection-state labels | Layout handles auth | Planned Google Workspace detail route exists with a coming-soon state; connected/disconnected/available labels are localized while run-result badges remain technical pass/warn/fail. Production browser check on 2026-05-09 confirmed signed-out redirect only; authenticated hub rendering still needs a tenant session. |
...
| Vendor detail | `/vendors/[vendorId]` | partial | `getVendorDetail`; demo only for `demo-cloud` | Demo detail read-only | Localized page copy | Org-scoped query; assessment/questionnaire writes now reject mismatched vendor ownership | Delivery-status panel is source/local-smoked but still needs authenticated production tenant smoke; production email deliverability is blocked until `RESEND_API_KEY` and `RESEND_FROM` are configured and a controlled mailbox receives the questionnaire email. |
...
| Trust Center admin | `/trust-center` | partial | `getTrustCenterSettings`; local-only fallback demo frameworks | Empty/unavailable state unless `ENABLE_LOCAL_DEMO_DATA=true` outside production | Localized | Layout handles auth | Signed-out production redirect, public demo, Splnit built-in Trust Center, unknown-slug 404, and demo framework detail category-only exposure were browser-verified on 2026-05-09. Saved slug, visibility toggles, NDA request approval, and framework-detail flags still need authenticated production tenant smoke; a direct 2026-05-09 smoke attempt did not run after local tool approval denied the smoke command. |
...
1. **Authenticated production tenant smoke.** Run `npm run smoke:production-tenant-readiness-prereqs` first, then run `npm run smoke:production-tenant-readiness` with production `DATABASE_URL`, `BLOB_READ_WRITE_TOKEN`, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, and `CLERK_SECRET_KEY` after rotating the previously exposed Clerk test secret and ensuring the Clerk variables have non-empty production values. The script should verify `/dashboard`, `/evidence`, `/integrations`, provider detail pages, `/trust-center`, `/vendors`, `/vendors/[vendorId]`, `/questionnaires`, a public Trust Center access URL, and a vendor assessment token URL without browser/page errors, then clean up the smoke tenant. Current 2026-05-09 smoke proves signed-out redirects, Clerk sign-in/signup rendering, public Trust Center routes, source/local smokes, Vercel env pull cleanup, and source availability of this reusable smoke; tenant rendering remains unproven because Vercel-pulled Clerk env names currently have empty values.
2. **Email deliverability setup.** Configure production `RESEND_API_KEY` and `RESEND_FROM`, then send a vendor questionnaire to a controlled mailbox and verify email arrival, token route open, assessment submit, vendor status update, and delivery-status accuracy.
...
4. **Outreach decision prep.** Treat the primary workflow as previously verified for outreach planning, while keeping authenticated questionnaire workbench, Italian policy-template promotion, and legal identity closeout out of demo claims until completed.

## Verification Needed Next

- Authenticated production tenant smoke for `/evidence`, `/questionnaires`, `/integrations`, `/trust-center`, `/vendors/[vendorId]`, and the questionnaire workbench/save flow. The reusable secondary-surface smoke command is now `npm run smoke:production-tenant-readiness`; it still needs to be run with non-empty production Clerk secrets. The 2026-05-09 browser pass verified signed-out redirects to Clerk sign-in with zero console errors; a later Vercel env pull verified DB/Blob values are available but Clerk env values currently pull empty, so the live smoke did not run.
- Production Resend mailbox smoke after `RESEND_API_KEY` and `RESEND_FROM` are configured; current production env metadata check did not show those variables.

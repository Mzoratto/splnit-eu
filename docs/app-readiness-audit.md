...
| Integrations hub | `/integrations` | partial | `getIntegrationsHubData`; provider config | Provider cards without DB; Google Workspace is disabled as coming soon | Localized descriptions and connection-state labels | Layout handles auth | Planned Google Workspace detail route exists with a coming-soon state; connected/disconnected/available labels are localized while run-result badges remain technical pass/warn/fail. |
...
| Trust Center admin | `/trust-center` | partial | `getTrustCenterSettings`; local-only fallback demo frameworks | Empty/unavailable state unless `ENABLE_LOCAL_DEMO_DATA=true` outside production | Localized | Layout handles auth | Signed-out production redirect, public demo, Splnit built-in Trust Center, unknown-slug 404, and demo framework detail category-only exposure were browser-verified on 2026-05-09. Saved slug, visibility toggles, NDA request approval, and framework-detail flags still need authenticated tenant smoke. |
...
1. **Authenticated production tenant smoke.** Sign in with a real production test tenant and verify `/evidence`, `/questionnaires`, questionnaire review/save/export gating, vendor questionnaire delivery-status panel, and Trust Center admin save/toggle/request flows render without browser console errors. Current 2026-05-09 smoke only proves signed-out redirects, Clerk sign-in/signup rendering, public Trust Center routes, and source/local smokes.

...
| Vendors | `/vendors` | partial | `listVendorsForOrg`; local-only demo cloud provider fallback | Empty/unavailable state unless `ENABLE_LOCAL_DEMO_DATA=true` outside production | Localized page copy | Layout handles auth; mutations disabled without live data | Category option values are raw English values; export endpoint auth needs smoke test. |

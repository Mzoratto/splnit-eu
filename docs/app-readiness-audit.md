...
- This verifies protected-route enforcement, sign-in/signup rendering, production deploy health, prerequisite presence for the second auth-smoke attempt, cleanup of the temporary Clerk env file, and source availability of the new reusable production tenant readiness smoke. It does **not** verify authenticated tenant rendering for `/evidence`, `/questionnaires`, `/integrations`, `/trust-center`, `/vendors/[vendorId]`, or production email delivery until that smoke runs successfully against production secrets.
...
1. **Authenticated production tenant smoke.** Run `npm run smoke:production-tenant-readiness` with production `DATABASE_URL`, `BLOB_READ_WRITE_TOKEN`, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, and `CLERK_SECRET_KEY` after rotating the previously exposed Clerk test secret. The script should verify `/dashboard`, `/integrations`, provider detail pages, `/trust-center`, `/vendors`, `/vendors/[vendorId]`, `/questionnaires`, a public Trust Center access URL, and a vendor assessment token URL without browser/page errors, then clean up the smoke tenant. Current 2026-05-09 smoke proves signed-out redirects, Clerk sign-in/signup rendering, public Trust Center routes, source/local smokes, prerequisite presence for a second auth-smoke attempt, and source availability of this reusable smoke; tenant rendering remains unproven until the command runs successfully.

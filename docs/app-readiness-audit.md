...
- A direct authenticated production smoke attempt was blocked earlier because the browser session was signed out, local `DATABASE_URL`, `CLERK_SECRET_KEY`, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, `RESEND_API_KEY`, and `RESEND_FROM` were unset, and `vercel env pull --environment=production` did not provide usable Clerk secret values locally even though production env metadata shows Clerk/DB/Blob variables are present.
...
- Production email delivery is also blocked at config level: `vercel env ls production` did not show `RESEND_API_KEY` or `RESEND_FROM`, so vendor questionnaire mailbox smoke must not be claimed.
- This verifies protected-route enforcement, sign-in/signup rendering, production deploy health, prerequisite presence for the second auth-smoke attempt, cleanup of the temporary Clerk env file, and source availability of the new reusable production tenant readiness smoke. `npm run smoke:production-tenant-readiness-prereqs` now provides a redacted readiness check; on this machine it reports `DATABASE_URL` and `BLOB_READ_WRITE_TOKEN` present, with Clerk, Resend, and smoke-recipient env missing. It does **not** verify authenticated tenant rendering for `/evidence`, `/questionnaires`, `/integrations`, `/trust-center`, `/vendors/[vendorId]`, or production email delivery until the live smoke runs successfully against production secrets.
...
1. **Authenticated production tenant smoke.** Run `npm run smoke:production-tenant-readiness-prereqs` first, then run `npm run smoke:production-tenant-readiness` with production `DATABASE_URL`, `BLOB_READ_WRITE_TOKEN`, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, and `CLERK_SECRET_KEY` after rotating the previously exposed Clerk test secret. The script should verify `/dashboard`, `/evidence`, `/integrations`, provider detail pages, `/trust-center`, `/vendors`, `/vendors/[vendorId]`, `/questionnaires`, a public Trust Center access URL, and a vendor assessment token URL without browser/page errors, then clean up the smoke tenant. Current 2026-05-09 smoke proves signed-out redirects, Clerk sign-in/signup rendering, public Trust Center routes, source/local smokes, prerequisite presence for a second auth-smoke attempt, and source availability of this reusable smoke; tenant rendering remains unproven until the command runs successfully.
...
- Authenticated production tenant smoke for `/evidence`, `/questionnaires`, `/integrations`, `/trust-center`, `/vendors/[vendorId]`, and the questionnaire workbench/save flow. The reusable secondary-surface smoke command is now `npm run smoke:production-tenant-readiness`; it still needs to be run with production secrets after Clerk secret rotation. The 2026-05-09 browser pass verified signed-out redirects to Clerk sign-in with zero console errors; a later local attempt confirmed required Clerk/DB/Blob env presence, but the older smoke command itself was denied by the local tool approval layer and did not run.

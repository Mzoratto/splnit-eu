...
- This verifies protected-route enforcement, sign-in/signup rendering, production deploy health, and missing local smoke credentials only. It does **not** verify authenticated tenant rendering for `/evidence`, `/questionnaires`, `/integrations`, `/trust-center`, `/vendors/[vendorId]`, or production email delivery.
...
| Trust Center admin | `/trust-center` | partial | `getTrustCenterSettings`; local-only fallback demo frameworks | Empty/unavailable state unless `ENABLE_LOCAL_DEMO_DATA=true` outside production | Localized | Layout handles auth | Signed-out production redirect, public demo, Splnit built-in Trust Center, unknown-slug 404, and demo framework detail category-only exposure were browser-verified on 2026-05-09. Saved slug, visibility toggles, NDA request approval, and framework-detail flags still need authenticated production tenant smoke; a direct 2026-05-09 attempt was blocked by missing usable production session/local Clerk secret. |
...
- Authenticated production tenant smoke for `/evidence`, `/questionnaires`, `/integrations`, `/trust-center`, `/vendors/[vendorId]`, and the questionnaire workbench/save flow. The 2026-05-09 browser pass verified signed-out redirects to Clerk sign-in with zero console errors; local runner auth creation is blocked until usable production Clerk/DB credentials or a real signed-in tenant session are available.

...
3. **Locale is uneven.** Most core pages read tenant locale or request locale. Billing still has hardcoded English copy and CZK formatting for all locales.
...
| Framework index | `/frameworks` | demo-risk | Static `FRAMEWORK_LIBRARY` | Always renders all frameworks | Uses request locale | Layout handles auth | Does not reflect tenant enrolled frameworks or plan limits. |
...
| Controls index | `/controls` | demo-risk | Static `CONTROL_LIBRARY` | Always renders library controls | CS gets Czech, EN/IT gets English title only | Layout handles auth | Not org-scoped; not status-aware; Italian control text is not localized. |
| Control detail | `/controls/[controlId]` | partial | `getControlDetailByKey`; falls back to static control | Forms disabled without DB detail | CS/EN only for control text | Layout handles auth; actions need separate review | Evidence upload blocked without Blob token; Italian descriptions fall back to English; verify action-level org checks. |
| Evidence vault | `/evidence` | partial | `listEvidenceVault` | Empty list when no DB/session/error | Tenant/request locale | Layout handles auth | Empty state is functional but needs browser check; export/download auth should be smoke-tested. |
| Policies index | `/policies` | partial | `listPoliciesForOrg`, resolved templates/source docs | Templates render even with no generated policies | Uses jurisdiction context | Layout handles auth | Template titles still use `titleCs` in card heading; generation blocked by Blob token; legal review status must be visible before customer use. |
| Policy detail | `/policies/[type]` | partial | Resolved template + org policies | Template renders with no generated versions | Uses jurisdiction context | Layout handles auth | Detail heading uses `template.titleCs`; verify IT/EN template resolution and source document fallback behavior. |

...
| Cleafy | Nicolò Pastore, Co-founder & CTO. Alternate: Carmine Giangregorio, Co-founder & Product Manager. | Partially. Cleafy has an official contact form; public `media@cleafy.com` is PR-only and should not be used for design-partner outreach. | Use manual LinkedIn route for Nicolò/Carmine if verified; otherwise use Cleafy's contact form with the owner name in the opening line. | Needs manual route choice before send. | Do not update tracker email with `media@`; only use contact form or verified LinkedIn route. |

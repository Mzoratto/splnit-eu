...
| Questionnaire AI | Processor for customer-provided questionnaire context; controller for service operation logs where applicable. | Draft answers grounded in customer controls, policies, and evidence references. | Customer organisation context, policies, evidence references, questionnaire contact data if included. | Anthropic, Neon, Vercel. | Generated answers as workspace data; vendor prompt retention per vendor terms/settings. | Confirm Anthropic data terms, opt-in wording, and whether sensitive evidence can be sent. |

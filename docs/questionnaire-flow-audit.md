...
- “Questionnaire review has been proven in production” until the extended production tenant readiness smoke has completed successfully with real production auth/env and its redacted output has been captured.
...
1. **Authenticated questionnaire review smoke.** Reviewer controls and export gating are implemented in source and covered by focused smokes. `npm run smoke:production-tenant-readiness` now includes a representative authenticated review/persistence path for a seeded generated questionnaire artifact; record the live production pass/fail output when production auth/env prerequisites are available.
